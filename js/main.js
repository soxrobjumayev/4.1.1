...
console.log(movies)
var myMovies = filterMovies(movies)
var myFilm = filterFilm(movies)
...
function rendarAll(){
    ...
    let moviesSelectEl = renderSelect(myMovies)
    moviesSelectEl.addEventListener("change",(event)=>{
        console.log(event.target.value)

        let foundedFilm = findFilm(event.target.value,myFilm)
        let filmSelectEl = renderSelectt(foundedFilm)
        containerEl.appendChild(filmSelectEl)
    })
    

    containerEl.appendChild(moviesSelectEl)

}


function findFilm(id,filmArr){
    let founded = []
    filmArr.forEach(value => value.id == id ? founded.push(value): '')
    return founded
}


function filterMovies(moviesArr){
    let tempMoviesArr = []

    moviesArr.forEach((value,index) => {
        tempMoviesArr.push({id:value.id,year:value.year})
    }) 
    return tempMoviesArr
}
function filterFilm(moviesArr){
    let tempMoviesArr = []

    moviesArr.forEach((value,index) => {
        tempMoviesArr.push({id:value.id,title:value.title})
    }) 
    return tempMoviesArr
}
...
function renderSelect(arr){
    ...
    optionEl.textContent = "film years"
    optionEl.disabled = true
    optionEl.selected = true
    arr.forEach((value,index) =>{
        let optionEl = document.createElement('option')
        optionEl.textContent = value.year
...
function renderSelectt(arr){
    ...
    arr.forEach((value,index) =>{
        let optionEl = document.createElement('option')
        optionEl.textContent = value.title+("title")
        optionEl.value = value.id
        parentSelectEl.appendChild(optionEl)
    })
    return parentSelectEl
